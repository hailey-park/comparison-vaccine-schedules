###################################################################################################
#Title: Non-Severe Waning VE Model
#Author: Hailey Park
#Date: July 25, 2023
###################################################################################################
...
waning_data <- read.csv("data/processed-data/nonsevere waning data absolute 120624 mean.csv") 
weights <- read.csv("data/processed-data/nonsevere waning data absolute 120624 weights.csv") 
waning_data_95ui <- read.csv("data/processed-data/nonsevere waning data absolute 120624 95ui.csv")
...
long_data_lower <- melt(waning_data_95ui %>% filter(X95.UI == "lower")) %>% filter(!is.na(value)) %>%
  mutate(months = as.numeric(substr(variable,6,8)),  #months
         prior_inf = ifelse(Prior.Infection == "Yes", 1, 0),
         ve =  value/100,
         num_doses = factor(Vaccine.Status, levels = c("Unvaccinated", "Boosted"), ordered = TRUE),
...
long_data_upper <- melt(waning_data_95ui %>% filter(X95.UI == "upper")) %>% filter(!is.na(value)) %>%
  mutate(months = as.numeric(substr(variable,6,8)),  #months
         prior_inf = ifelse(Prior.Infection == "Yes", 1, 0),
         ve =  value/100, 
         num_doses = factor(Vaccine.Status, levels = c("Unvaccinated", "Boosted"), ordered = TRUE),
...
nonsevere_model <- lm(ve_input ~ month_input  + factor(age_group) + estimate ,
                      weights = weight,
...
#       we are creating predictions for each model/immunity type separately, so will need to select the correct "prior_inf" value according
#       to immunity type.
...
preds_weekly <- predict(nonsevere_model, newdata = prediction_data_weekly, allow.new.levels = TRUE)
...
write.csv(prediction_data_immunocompetent, "data/clean-data/nonsevere_waning_predictions_weekly_infectionImmunityOnly.csv")
# write.csv(prediction_data_immunocompetent, "data/clean-data/nonsevere_waning_predictions_weekly_vaccineImmunityOnly.csv")
# write.csv(prediction_data_immunocompetent, "data/clean-data/nonsevere_waning_predictions_weekly_hybridImmunityOnly.csv")
...
infection_immunocompromised <- read.csv("data/clean-data/nonsevere_waning_predictions_weekly_infectionImmunityOnly.csv")[,-1] %>% rowwise() %>%
  mutate(ve_pred = max(0, ve_pred - 0.15))
vaccine_immunocompromised <- read.csv("data/clean-data/nonsevere_waning_predictions_weekly_vaccineImmunityOnly.csv")[,-1] %>% rowwise() %>%
  mutate(ve_pred = max(0, ve_pred - 0.15))
hybrid_immunocompromised <- read.csv("data/clean-data/nonsevere_waning_predictions_weekly_hybridImmunityOnly.csv")[,-1] %>% rowwise() %>%
  mutate(ve_pred = max(0, ve_pred - 0.15))

# write.csv(infection_immunocompromised, "data/clean-data/nonsevere_waning_predictions_weekly_infectionImmunityOnly_immunocompromised.csv")
# write.csv(vaccine_immunocompromised, "data/clean-data/nonsevere_waning_predictions_weekly_vaccineImmunityOnly_immunocompromised.csv")
# write.csv(hybrid_immunocompromised, "data/clean-data/nonsevere_waning_predictions_weekly_hybridImmunityOnly_immunocompromised.csv")


#Plot curves (read in correct waning curve file based on immunity type and immunocompromised status)
plot_data <- read.csv("data/clean-data/nonsevere_waning_predictions_weekly_hybridImmunityOnly_immunocompromised.csv")[,-1]
...
plot_data %>% 
  ...
  ggtitle("Hybrid immunity waning curves against general COVID-19 infection\nAge Group: 65+ years\nRisk Group: Immunocompromised")
