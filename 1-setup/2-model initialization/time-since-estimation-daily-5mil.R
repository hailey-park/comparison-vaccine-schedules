###################################################################################################
#Title: 'Time Since Last' Estimation (5 million population)
#Author: Hailey Park
#Date: June 19, 2024
###################################################################################################
...
age_0_17_specs <- data.frame(prior_vacc=c("unvax","unvax","fullvax", "fullvax", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "fullvax"), 
                             ...
                             ntimes=c(0.086001, 0.332, 0.121, 0.3, 0.066 * (1-0.476), 0.066 * 0.476, 0.074 * (1-0.476), 0.074 * 0.476004, 0.009 * (1-0.476), 0.009 * 0.47601, 0.012) * (pop_size*0.2176))

sum(floor(age_0_17_specs$ntimes))
pop_size*0.2176

age_0_17_cal <- as.data.frame(lapply(age_0_17_specs[,1:2], rep, age_0_17_specs$ntimes)) %>%
  mutate(individual = c(1:(pop_size*0.2176)),
         age_group = "0-17 years",
         risk_group = sample(c('healthy', 'immunocompromised', 'higher risk'), (pop_size*0.2176), prob = c(0.882, 0.026, 0.092), replace = TRUE))
...
age_18_29_specs <- data.frame(prior_vacc=c("unvax","unvax","fullvax", "fullvax", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "fullvax"), 
                              prior_inf=c("inf","reinf","inf","reinf","inf", "inf", "reinf", "reinf", "noinf", "noinf", "noinf"), 
                              ntimes=c(0.046, 0.177002, 0.1, 0.246, 0.175 * (1-0.388), 0.175 * 0.388, 0.194 * (1-0.388), 0.194 * 0.38801, 0.043 * (1-0.388), 0.043 * 0.388, 0.019) * (pop_size*0.157))

age_18_29_cal <- as.data.frame(lapply(age_18_29_specs[,1:2], rep, age_18_29_specs$ntimes)) %>%
  mutate(individual = c((1 + (pop_size*0.2176)):((pop_size*0.3746))),
         age_group = "18-29 years",
         risk_group = sample(c('healthy', 'immunocompromised', 'higher risk'), (pop_size*0.157), prob = c(0.726, 0.033, 0.241), replace = TRUE))
...
age_30_49_specs <- data.frame(prior_vacc=c("unvax","unvax","fullvax", "fullvax", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "fullvax"), 
                              prior_inf=c("inf","reinf","inf","reinf","inf", "inf", "reinf", "reinf", "noinf", "noinf", "noinf"), 
                              ntimes=c(0.026001, 0.101001, 0.074001, 0.185001, 0.232 * (1-0.271), 0.232 * 0.271, 0.258 * (1-0.271), 0.258 * 0.271001, 0.1 * (1-0.271), 0.1 * 0.271, 0.024) * (pop_size*0.2621))

age_30_49_cal <- as.data.frame(lapply(age_30_49_specs[,1:2], rep, age_30_49_specs$ntimes)) %>%
  mutate(individual = c((1 + (pop_size*0.3746)):((pop_size*0.6367))),
         age_group = "30-49 years",
         risk_group = sample(c('healthy', 'immunocompromised', 'higher risk'), (pop_size*0.2621), prob = c(0.726, 0.056, 0.218), replace = TRUE))
...
age_50_64_specs <- data.frame(prior_vacc=c("unvax","unvax","fullvax", "fullvax", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "fullvax"), 
                              prior_inf=c("inf","reinf","inf","reinf","inf", "inf", "reinf", "reinf", "noinf", "noinf", "noinf"), 
                              ntimes=c(0.0220, 0.0850, 0.048003, 0.118, 0.274 * (1-0.420), 0.274 * 0.420003, 0.304 * (1-0.420), 0.304 * 0.420, 0.131 * (1-0.420), 0.131 * 0.420, 0.018) * (pop_size*0.1868))

age_50_64_cal <- as.data.frame(lapply(age_50_64_specs[,1:2], rep, age_50_64_specs$ntimes)) %>%
  mutate(individual = c((1 + (pop_size*0.6367)):((pop_size*0.8235))),
         age_group = "50-64 years",
         risk_group = sample(c('healthy', 'immunocompromised', 'higher risk'), (pop_size*0.1868), prob = c(0.366, 0.089, 0.545), replace = TRUE))
...
age_65_74_specs <- data.frame(prior_vacc=c("unvax","unvax","fullvax", "fullvax", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "fullvax"), 
                              prior_inf=c("inf","reinf","inf","reinf","inf", "inf", "reinf", "reinf", "noinf", "noinf", "noinf"), 
                              ntimes=c(0.016008, 0.0600, 0.035, 0.086, 0.272 * (1-0.686), 0.272 * 0.6860, 0.303 * (1-0.686), 0.303 * 0.686, 0.208 * (1-0.686), 0.208 * 0.686, 0.02) * (pop_size*0.1035))

age_65_74_cal <- as.data.frame(lapply(age_65_74_specs[,1:2], rep, age_65_74_specs$ntimes)) %>%
  mutate(individual = c((1 + (pop_size*0.8235)):((pop_size*0.927))),
         age_group = "65-74 years",
         risk_group = sample(c('healthy', 'immunocompromised', 'higher risk'), (pop_size*0.1035), prob = c(0.124, 0.1065, 0.7695), replace = TRUE))
...
age_75plus_specs <- data.frame(prior_vacc=c("unvax","unvax","fullvax", "fullvax", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "boosted_1st", "boosted_2nd", "fullvax"), 
                               prior_inf=c("inf","reinf","inf","reinf","inf", "inf", "reinf", "reinf", "noinf", "noinf", "noinf"), 
                               ntimes=c(0.016008, 0.0600, 0.0350, 0.0860, 0.272 * (1-0.736), 0.272 * 0.73601, 0.303 * (1-0.736), 0.303 * 0.736008, 0.208 * (1-0.736), 0.208 * 0.736, 0.02) * (pop_size*0.073))

age_75plus_cal <- as.data.frame(lapply(age_75plus_specs[,1:2], rep, age_75plus_specs$ntimes)) %>%
  mutate(individual = c((1 + (pop_size*0.927)):(pop_size)),
         age_group = "75+ years",
         risk_group = sample(c('healthy', 'immunocompromised', 'higher risk'), (pop_size*0.073), prob = c(0.124, 0.1065, 0.7695), replace = TRUE))
...
write.csv(combined, "data/clean-data/entire_population_model_initialization_daily_5mil_updated.csv")
############################################################
#PLOTS OF TIME-SINCE EVENT DISTRIBUTION FOR ENTIRE SIMULATED POPULATION

pdf("time-since-plots-5mil.pdf")
pop_data <- read.csv("data/clean-data/entire_population_model_initialization_daily_5mil_updated.csv")[,-1] #%>%
...
for(i in c(1:6)) {
  ...
  inspection <- pop_data %>% 
    filter(age_group == age_group_label) %>% 
    dplyr::select(prior_vacc, prior_inf, time_since_last_dose, time_since_last_inf, time_since_last_reinf, time_since_last_dose_inf) %>%
    group_by(prior_vacc, time_since_last_dose) %>% 
    summarise(total = n()) %>% 
    filter(prior_vacc != 'unvax')
  
  observed_data <- as.data.frame(rbind(fully_vax_doses_by_day %>% 
                                         mutate(prior_vacc = "observed_fullvax"),
                                       booster_doses_1st_by_day %>% 
                                         mutate(prior_vacc = "observed_boosted_1st"),
                                       booster_doses_2nd_by_day %>% 
                                         mutate(prior_vacc = "observed_boosted_2nd"))) %>% 
    dplyr::select(prior_vacc, day, age_group, perc_doses) %>% 
    filter(age_group == age_group_label)
  ...
  inspection <- pop_data %>% 
    filter(age_group == age_group_label) %>% 
    dplyr::select(prior_vacc, prior_inf, time_since_last_dose, time_since_last_inf, time_since_last_reinf, time_since_last_dose_inf) %>%
    group_by(time_since_last_inf) %>% 
    summarise(total = n()) %>% 
    filter(!is.na(time_since_last_inf))
  ...
  inspection <- pop_data %>% 
    filter(age_group == age_group_label) %>% 
    dplyr::select(prior_vacc, prior_inf, time_since_last_dose, time_since_last_inf, time_since_last_reinf, time_since_last_dose_inf) %>%
    group_by(time_since_last_reinf) %>% 
    summarise(total = n()) %>% 
    filter(!is.na(time_since_last_reinf))
  ...
  inspection <- pop_data %>% 
    filter(age_group == age_group_label) %>% 
    dplyr::select(prior_vacc, prior_inf, time_since_last_dose, time_since_last_inf, time_since_last_dose_inf) %>%
    group_by(time_since_last_dose_inf) %>% 
    summarise(total = n()) 
