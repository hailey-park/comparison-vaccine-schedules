...
vaccine_dec2024 <- read.csv("data/raw-data/Weekly_Cumulative_COVID-19_Vaccination_Coverage_and_Intent__Overall__by_Selected_Demographics_and_Jurisdiction__Among_Adults_18_Years_and_Older_20250107.csv")
vaccine_dec2024_child <- read.csv("data/raw-data/Weekly_Parental_Intent_for_Vaccination_and_Cumulative_Percentage_of_Children_6_Months_-17_Years_Who_are_Up_to_date_with_the_COVID-19_Vaccines_by_Season__United_States_20250107.csv")
...
vaccines_risk <- read.csv("data/raw-data/National_Immunization_Survey_Adult_COVID_Module__NIS-ACM___COVIDVaxViews__Data___Centers_for_Disease_Control_and_Prevention__cdc.gov__20250114.csv")
...
prevalence_adj <- prevalence %>% 
  mutate(high_risk = if_else(risk_group != "healthy", "Yes", "No")) %>%
...
combined_with_risk <- rbind(healthy_vax_coverage,
  ...
  filter(week != 0) %>%
  dplyr::select(date, week, age_group, risk_group, risk_coverage, prop_up_to_date_sim_period)


#write.csv(combined_with_risk, "data/clean-data/vaccine-coverage-by-age-and-risk.csv")

plot_data <- melt(combined_with_risk, id = c("age_group", 'risk_group',  'date'))

plot_data %>% 
  filter(variable == "risk_coverage", age_group == "30-49 years") %>%
...
#2nd dose uptake (see screenshot in timed-booster slack from nathan - ref: National Immunization Survey-Adult COVID Module)
second_dose_annually <- data.frame(week = rep(seq(as.Date('2024-03-02'), as.Date("2024-07-01"), by = "weeks"), 6 * 3),
  ...
  group_by(age_group, risk_group) %>% 
  mutate(coverage = case_when(age_group %in% c("65-74 years", "75+ years") ~ c(cumsum(rep(3.8/4,4)), 3.8 + cumsum(rep((5.7 - 3.8)/5,5)), 5.7 + cumsum(rep((7.3 - 5.7)/4,4)), 7.3 + cumsum(rep((8.9 - 7.3)/5,5))),
                              risk_group != "healthy" ~ c(cumsum(rep(2.5/4,4)), 2.5 + cumsum(rep((3.3 - 2.5)/5,5)), 3.3 + cumsum(rep((4.5 - 3.3)/4,4)), 4.5 + cumsum(rep((5.4 - 4.5)/5,5))),
                              TRUE ~ 0),
         perc_up_to_date_week = c((coverage - lag(coverage, 1))[2], (coverage - lag(coverage, 1))[-1])) %>% 
  rowwise() %>%
  mutate(perc_up_to_date_week = if_else(perc_up_to_date_week < 0, coverage, perc_up_to_date_week)) %>%
  group_by(age_group, risk_group) %>%
  mutate(prop_up_to_date_sim_period = perc_up_to_date_week/sum(perc_up_to_date_week),
         week = as.numeric(interval(as.Date('2023-07-01'), week) %/% weeks(1))) %>%
  filter(week != 0) %>%
  dplyr::select(week, age_group, risk_group, coverage, prop_up_to_date_sim_period)

#write.csv(second_dose_annually, "data/clean-data/2nd-dose-annually-vaccine-coverage-by-age-and-risk.csv")                                   

plot_data <- melt(second_dose_annually, id = c("age_group", 'risk_group',  "week"))

plot_data %>% filter(variable == "coverage", age_group == "75+ years") %>%
  ggplot(aes(week, value, color = interaction(age_group, risk_group))) + 
