...
# Load in data
covidnet_char_data <- read.csv("data/raw-data/Patient_Characteristics_of_Laboratory-Confirmed_COVID-19_Hospitalizations_from_the_COVID-NET_Surveillance_System_20250321.csv")
covidnet_data <- read.csv("data/raw-data/Weekly_Rates_of_Laboratory-Confirmed_COVID-19_Hospitalizations_from_the_COVID-NET_Surveillance_System_20250322.csv")
...
average_data_2024 <- combined_char_data %>% 
  filter(year == 2024, age_group != "Overall") %>%
  group_by(age_group) %>% 
  summarise(hosp_reason_estimate = mean(hosp_reason_estimate),
...
write.csv(adj_severe_inc_clean %>% 
          ...
          "data/clean-data/weekly-incidence-estimates-US-validationPeriod.csv") # FIXME -- remove _2 if not using CF
...
write.csv(adj_severe_inc_clean_model_input %>% dplyr::select(week, weeks_since, age_group, adj_inc, healthy_inc, immunocompromised_inc, higher_risk_inc),
          "data/clean-data/weekly-incidence-estimates-US-validationPeriod-modelInput.csv")
...
p1 <- plot_data %>% 
  ...
  scale_x_date(date_labels = ("%b-%Y"),
               limits = c(as.Date("2023-07-01"), as.Date("2025-01-01")),
               breaks = "1 months") +
