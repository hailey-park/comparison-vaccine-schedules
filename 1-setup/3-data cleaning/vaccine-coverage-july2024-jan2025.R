...
vaccine_2025 <- read.csv("data/raw-data/Weekly_Cumulative_COVID-19_Vaccination_Coverage_and_Intent__Overall__by_Selected_Demographics_and_Jurisdiction__Among_Adults_18_Years_and_Older_20250321.csv")
vaccine_2025_child <- read.csv("data/raw-data/Weekly_Parental_Intent_for_Vaccination_and_Cumulative_Percentage_of_Children_6_Months_-17_Years_Who_are_Up_to_date_with_the_COVID-19_Vaccines_by_Season__United_States_20250321.csv")
...
vaccines_risk <- read.csv("data/raw-data/National_Immunization_Survey_Adult_COVID_Module__NIS-ACM___RespVaxView__Data___Centers_for_Disease_Control_and_Prevention__cdc.gov__20250321.csv")
...
doses_by_month_mar2025_adult <- vaccine_2025 %>% 
  filter(Geographic.Level == "National", Demographic.Level == "Age", indicator_label == "Up-to-date", Demographic.Name %in% c("18-29 years", "30-39 years", "40-49 years", "50-64 years", "65-74 years", "75+ years")) %>%
  mutate(date = as.Date(Week_ending, format = "%m/%d/%Y"),
         age_group = if_else(Demographic.Name %in% c("30-39 years", "40-49 years"), "30-49 years", Demographic.Name)) %>% 
  group_by(date, age_group) %>%
  summarise(perc = mean(Estimate)) %>%
  group_by(age_group) %>% 
  mutate(perc_up_to_date_week = c((perc - lag(perc, 1))[2], (perc - lag(perc, 1))[-1])) %>% 
  rowwise() %>%
  mutate(perc_up_to_date_week = if_else(perc_up_to_date_week < 0, perc, perc_up_to_date_week)) %>%
  filter(date >= as.Date("2024-06-29") & date <= as.Date("2025-01-01")) %>% 
  dplyr::select(date, age_group, perc_up_to_date_week) %>%
  group_by(age_group)

#Vaccine doses by month from October 2024 through January 2025 (CHILDREN)
doses_by_month_mar2025_child <- vaccine_2025_child %>% 
  filter(Geographic.Level == "National", Demographic_Level == "Age", Indicator_label == "Up-to-date", Demographic.Name %in% c("6 months-4 years", "5-17 years")) %>%
  ...
  filter(date >= as.Date("2024-7-01") & date <= as.Date("2025-01-01")) %>% 
  dplyr::select(date, age_group, perc_up_to_date_week) %>%
  group_by(age_group) 
