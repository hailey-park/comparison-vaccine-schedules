###################################################################################################
#Title: Model setup (load each-time)
#Author: Hailey Park
...
weekly_nonsevere_incidence <- merge(weekly_severe_incidence, age_ratio_case_hosp_frac, by = "age_group", all.x = TRUE) %>%
  dplyr::mutate(healthy_inc = adj_inc * (1/(ratio * baseline_case_hosp_frac)),
...
contact_matrix_adj <- data.frame(age_group = c("0-17 years","18-29 years", "30-49 years", "50-64 years", "65-74 years", "75+ years"),
                                 contact_matrix_adj = (sum(inf_by_age$total_inf)/1000000)/c(sum((inf_by_age$total_inf/inf_by_age$total_pop) * contact_matrix$X0.17.years),
...
clean_age_matrix <- function(df){
  df %>% dplyr::select(c("individual", "age_group", "prior_inf", "risk_group","time_since_last_dose_inf", "time_since_last_dose", "prior_vacc", "vaccine_wave", "second_vaccine_wave", "vaccine_wave_val")) %>%
...
clean_df <- list(entire_pop_with_vax_assigned) %>%
