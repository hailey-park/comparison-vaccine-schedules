...
#Load in data
covidnet_char_data <- read.csv("raw-data/Clinical_Characteristics_of_Laboratory-Confirmed_COVID-19_Hospitalizations_from_the_COVID-NET_Surveillance_System_20241122.csv")
covidnet_data <- read.csv("raw-data/Weekly_Rates_of_Laboratory-Confirmed_COVID-19_Hospitalizations_from_the_COVID-NET_Surveillance_System_20241204.csv")
...
#         Link: https://data.cdc.gov/Public-Health-Surveillance/Weekly-Rates-of-Laboratory-Confirmed-COVID-19-Hosp/twtx-bfcw/about_data
...
# #Clean prevalence data (broad risk group)
# prevalence_clean <- prevalence_data %>%
#   mutate(broad_risk_group = if_else(risk_group != "healthy", "underlying_cond", risk_group)) %>%
#   group_by(broad_risk_group, age_group) %>% mutate(broad_risk_group_prev = sum(prevalence)) %>%
#   dplyr::select(age_group, broad_risk_group, broad_risk_group_prev) %>%
#   unique() %>% pivot_wider(names_from = broad_risk_group,
#                            values_from = broad_risk_group_prev) %>%
#   mutate(age_group_prev = healthy + underlying_cond)
# 
# 
# #Combine severe inc data with patient characteristic info
# combined_inc_with_char <- merge(merge(severe_inc_clean, combined_char_data, by = c("age_group", "month", "year"), all.x = TRUE),
...
#          underlying_cond_inc = underlying_cond_estimate/100 * adj_inc * age_group_prev/underlying_cond,
#          healthy_inc = (1 - underlying_cond_estimate/100) * adj_inc * age_group_prev/healthy) %>%
#   dplyr::select(age_group, week, weeks_since, inc, adj_inc, underlying_cond_inc, healthy_inc)
# 
# 
# write.csv(adj_severe_inc_clean %>% dplyr::select(weeks_since, age_group, adj_inc, underlying_cond_inc, healthy_inc), "data/clean-data/weekly-incidence-estimates-US.csv")
# 
...
#write.csv(adj_severe_inc_clean %>% dplyr::select(weeks_since, age_group, adj_inc, healthy_inc, immunocompromised_inc, higher_risk_inc), "data/clean-data/weekly-incidence-estimates-US.csv")
