...
###################################################################################################
#Cases by Week
cases_by_week <- cases %>% mutate(date = as.Date(date)) %>% 
...
ggplot(data = cases_by_week, aes(x = date, y = total_cases)) +
  geom_point() + geom_line()
################################################################################################### 
#Vaccine Doses by Week (for primary series and 1st booster coverage, 2nd booster coverage will be calculated later)
doses_by_week <- vaccines_may2023 %>% 
...
#Estimating % of people who completed their 1st booster dose at each week, from December 15, 2020 - May 10, 2023
...
booster_doses_by_week <- doses_by_week %>% filter(date >= as.Date("2021-09-22")) %>%
  ...
  dplyr::select(week, age_group, perc_doses) %>%
  mutate(age_group = if_else(age_group == "18-49 years", "18-29 years", age_group))

age_30_49_only <- booster_doses_by_week %>% filter(age_group == "18-29 years") %>% mutate(age_group = "30-49 years")
booster_doses_by_week_final <- data.frame(rbind(booster_doses_by_week, age_30_49_only))

# Estimating the % of people who completed primary series at each week, from December 15, 2020 - May 10, 2023. 
...
fully_vax_doses_by_week <- doses_by_week %>% 
...
age_30_49_only <- fully_vax_doses_by_week %>% filter(age_group == "18-29 years") %>% mutate(age_group = "30-49 years")
fully_vax_doses_by_week_final <- data.frame(rbind(fully_vax_doses_by_week, age_30_49_only))

################################################################################################### 
#Vaccine doses by week through May 2023 (2nd boosters monovalent)
doses_by_week_may2023 <- vaccines_may2023 %>% 
  ...
  group_by(age_group) %>% mutate(week_num = as.numeric(difftime(week, as.Date("2021-12-26"), units = "weeks"))) %>%
...
age_ratio_1st_booster <- doses_by_week_may2023 %>% filter(week == as.Date("2023-05-07"), !age_group %in% c("65-74 years", "75+ years")) %>%
  mutate(ratio = (total_boosted_1st/total_pop)/(4351705/7250961)) %>%
  dplyr::select(age_group, ratio)

imputed_younger_age_coverage_data <- melt(doses_by_week_may2023 %>% mutate(coverage = (total_boosted_2nd)/(total_pop)) %>%
  filter(!age_group %in% c("65-74 years", "75+ years")) %>%
  dplyr::select(age_group, week, coverage) %>% 
  pivot_wider(names_from = age_group,
              values_from = coverage) %>%
  rowwise() %>%
  mutate(`0-17 years` = max(`50-64 years` * (age_ratio_1st_booster %>% filter(age_group == "0-17 years"))$ratio - 0.006, 0),
         `18-49 years` = max(`50-64 years` * (age_ratio_1st_booster %>% filter(age_group == "18-49 years"))$ratio - 0.05, 0)),
  id = "week") %>% rename(age_group = variable, coverage = value)
...
missing_data <- data.frame(week = rep(seq(as.Date("2023-05-14"), as.Date("2023-07-01"), by = "weeks"), 5),
                           age_group = rep(c("0-17 years", "18-49 years", "50-64 years", "65-74 years", "75+ years"), each = 7),
                           coverage = c(c(0.068 + c(1:7) * (0.071 - 0.068)/7), #0-17 years
                                        c(0.157 + c(1:7) * (0.16 - 0.157)/7), #18-49 years
                                        rep(0.298, 7), #50-64 years
                                        c(0.495 + c(1:7) * (0.537 - 0.495)/7), #65-74 years
                                        c(0.504 + c(1:7) * (0.576 - 0.504)/7))) #75+ years

#Combine imputed data together
combined_coverage_data <- merge(merge(doses_by_week_may2023, imputed_younger_age_coverage_data, by = c("age_group", "week"), all.x = TRUE),
                                missing_data, by = c("age_group", "week"), all = TRUE) %>%
...
#Estimating the % of people who get a 2nd booster each week
booster_doses_2nd_by_week <- combined_coverage_data %>%
  group_by(age_group) %>%
...
age_30_49_only <- booster_doses_2nd_by_week %>% filter(age_group == "18-29 years") %>% mutate(age_group = "30-49 years")
booster_doses_2nd_by_week_final <- data.frame(rbind(booster_doses_2nd_by_week, age_30_49_only))


################################################################################################### 
#write as .csv
# write.csv(cases_by_week, "data/clean-data/cases_by_week.csv")
# write.csv(booster_doses_by_week_final, "data/clean-data/booster_1st_doses_by_week.csv")
# write.csv(booster_doses_2nd_by_week_final, "data/clean-data/booster_2nd_doses_by_week.csv")
# write.csv(fully_vax_doses_by_week_final, "data/clean-data/fully_vax_doses_by_week.csv")

#Plot coverages over time
ggplot(data = doses_by_week, aes(x = date, y = total_boosted_1st/total_pop, color = age_group)) +
  geom_line() +
  ylim(0, 1) +
