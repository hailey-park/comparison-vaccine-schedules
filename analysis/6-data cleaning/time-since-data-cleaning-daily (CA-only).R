...
missing_data <- data.frame(day = rep(seq(as.Date("2023-05-11"), as.Date("2023-07-01"), by = "days"), 5),
                           ...
                           coverage = c(c(0.068 + c(1:52) * (0.071 - 0.068)/52), #0-17 years
                                        c(0.157 + c(1:52) * (0.16 - 0.157)/52), #18-49 years
                                        rep(0.298, 52), #50-64 years
                                        c(0.495 + c(1:52) * (0.537 - 0.495)/52), #65-74 years
                                        c(0.504 + c(1:52) * (0.576 - 0.504)/52))) #75+ years
...
# write.csv(booster_doses_2nd_by_day_final, "data/clean-data/booster_2nd_doses_by_day.csv")
# write.csv(fully_vax_doses_by_day_final, "data/clean-data/fully_vax_doses_by_day.csv")

#Plot coverages over time
ggplot(data = booster_doses_2nd_by_day_final, aes(x = date, y = total_boosted_2nd/total_pop, color = age_group)) +
  geom_line() +
  ylim(0, 1) +
