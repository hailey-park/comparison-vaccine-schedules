...
simulation_annual <- function(df){
  ...
  for (i in (1:547)) {
    ...
    input$vax_day1 <- as.integer(as.logical(((vaccine_wave < i & vaccine_wave != 0) | (second_vaccine_wave < i & second_vaccine_wave != 0) | (third_vaccine_wave < i & third_vaccine_wave != 0)) & severe_outcomes == 1 & input$vax_status == 1))
    input$vax_nonsevere_day1 <- as.integer(as.logical(((vaccine_wave < i & vaccine_wave != 0) | (second_vaccine_wave < i & second_vaccine_wave != 0) | (third_vaccine_wave < i & third_vaccine_wave != 0)) & nonsevere_outcomes == 1 & input$vax_status == 1))
    ...
    grouped_outcomes <- input %>% 
      group_by(age_group, risk_group) %>% summarise(total_severe = sum(day1),
                                                    ...
                                                    total_vaccines = sum(vax) + sum(vax_2) + sum(vax_3),
