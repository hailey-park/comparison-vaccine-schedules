...
############################################################################################
#Beta calculation: These are fixed relationships between different age and risk groups based on nonsevere incidence and the 
# (1 - PE) estimate. We are assuming that the 0-17 year old healthy group is the reference group. We are using the average 
# nonsevere incidence and average (1-PE) estimates by age group at t = 0 to calculate beta.

beta_ratios <- merge(protection_at_model_init[[4]], average_nonsevere_incidence, by = c("age_group", "risk_group"), all.x = TRUE) %>%
  mutate(pe_adj = (1 - mean_nonsevere_ve[1])/(1 - mean_nonsevere_ve),
         inc_adj = nonsevere_inc/nonsevere_inc[1],
         beta_ratio = pe_adj * inc_adj) %>%
  dplyr::select(age_group, risk_group, beta_ratio, mean_nonsevere_ve)


#Beta ~contact matrix~ factors calculation: Contact matrix adjustment @ t = 0 (These are age-specific adjustments to account for differences in magnitude from including social mixing)
beta_contact_matrix <- data.frame(age_group = c("0-17 years","18-29 years", "30-49 years", "50-64 years", "65-74 years", "75+ years"),
...
#Beta ~lambda~ calculation: This function analytically calculates the expected lambda at model initialization assuming average
# nonsevere incidence and average protection estimates at t = 0. This will get multiplied back with the beta ratios to set 
# the "official" betas for this model
set_beta_lambda <- function() {
  ...
  lambda <- sum((avg_nonsevere_inc_adj)$nonsevere_inc)/sum(beta_ratios$beta_ratio * (1-beta_ratios$mean_nonsevere_ve) * (total_inf/50000000))

  return(lambda)
}

beta_lambda <- set_beta_lambda()

#Merge analytically-calculated lambda and multiply with the beta ratios and beta contact matrix factors to set "official" betas.
beta <- setDT(merge(beta_ratios, beta_contact_matrix, by = "age_group", all.x = TRUE) %>%
  mutate(lambda = beta_lambda,
         beta = beta_ratio * lambda * contact_matrix_adj)%>%
