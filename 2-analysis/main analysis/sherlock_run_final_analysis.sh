...
#SBATCH --job-name=main_analysis      # Job name
#SBATCH --mail-type=ALL               # Mail events (NONE, BEGIN, END, FAIL, ALL)
#SBATCH --mail-user=kbubar@stanford.edu # Where to send mail  
#SBATCH --nodes=1                     # Use one node
#SBATCH --ntasks=1                    # Run a single task
#SBATCH --mem-per-cpu=80gb            # Memory per processor
#SBATCH --time=6:00:00                # Time limit hrs:min:sec
#SBATCH --output=array_%A-%a.out      # Standard output and error log
#SBATCH --array=1-220                 # Array range
...
Rscript run-analysis.R ${SLURM_ARRAY_TASK_ID}
