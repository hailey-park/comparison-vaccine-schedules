...
# Read in population df (correponding to clean_df) that has vaccines allocated
df_sim <- readRDS("data/clean-data/df_sim_5mil.RDS")
...
average_severe_incidence <- weekly_severe_incidence %>%
  ...
  filter(weeks_since < time_avg_over) %>% 
...
assign_protection_at_model_initialization <- function(df) {
...
protection_at_model_init <- assign_protection_at_model_initialization(clean_df[[1]]) 
