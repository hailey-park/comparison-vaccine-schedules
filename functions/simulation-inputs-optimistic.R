...
#Optimistic vaccine coverage by age and risk group. Calculated proportion of population who receive vaccine each week of simulation period 
first_dose_coverage <- read.csv("data/clean-data/vax-uptake-scenarios/optimistic-vaccine-coverage-by-age-and-risk.csv")[,-1] 
second_dose_coverage <- read.csv("data/clean-data/vax-uptake-scenarios/optimistic-2nd-dose-annually-vaccine-coverage-by-age-and-risk.csv")[,-1]
next_year_dose_data <- NA
...
# UPDATED: HJP: can you check the format of these df
...
average_nonsevere_incidence <- weekly_nonsevere_incidence %>%
  ...
  mutate(
    risk_group = sub("_.*", "", variable),                     # prefix before "_"
    risk_group = if_else(risk_group == "higher", "higher risk", risk_group)
  ) %>%
  select(-variable)
...
beta_analytic_calibration <- function() {
  ...
  pop_adj_severe_infection_multipliers <- (merge(severe_infection_multipliers %>% 
  ...
  for(i in c(1:6)) {
    beta_vector[i] <- severe_risk_and_protection$severe_risk[i] /
      ((1 - severe_risk_and_protection$mean_severe_ve[i]) * dynamic_term[i] * pop_adj_severe_infection_multipliers[i])
